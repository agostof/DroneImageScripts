...
print("Running optimized ZoneStats calculation")
...
for index, row in input_image_file_data.iterrows():
    ...
    for image in images_array:
        ...
        # vectorized 
        total_pixel_sum_v2 = np.sum(img)
        pixel_array_v2 = img.flatten()
        unique, counts = np.unique(pixel_array_v2, return_counts=True)
        pixel_dict_v2 = dict(zip(unique, counts))

        # assert total_pixel_sum == total_pixel_sum_v2
        # assert list(pixel_array_v2) == list(pixel_array)
        # assert sorted(list(pixel_dict_v2.values())) == sorted(list(pixel_dict.values()))
        # assert  statistics.variance(pixel_array_v2.astype(float)) == statistics.variance(pixel_array)
        total_pixel_sum = total_pixel_sum_v2
        # pixel_array_orig = pixel_array
        pixel_array = pixel_array_v2
        pixel_dict = pixel_dict_v2

        #print("Total: %s" % total_pixel_sum)

        #mean_pixel_value = statistics.mean(pixel_array_orig)
        mean_pixel_value = np.mean(pixel_array)
        #print("Mean: %s" % mean_pixel_value)

        # equivalent numpy and statistics stdev
        #assert statistics.stdev(pixel_array_orig) == np.std(pixel_array_orig, ddof=1)
        #assert statistics.variance(pixel_array_orig) == np.var(pixel_array_orig, ddof=1)
        #assert statistics.pstdev(pixel_array_orig) == np.std(pixel_array_orig, ddof=0)
        #assert statistics.mean(pixel_array_orig) == np.mean(pixel_array_orig)
        ...
        if np.any(pixel_array == 0):
            harmonic_mean_pixel_value = 0
        else:
            harmonic_mean_pixel_value = (len(pixel_array) / np.sum(1.0 / pixel_array))
        #assert np.allclose(harmonic_mean_pixel_value, (len(pixel_array) / np.sum(1.0 / pixel_array)))

        #print("Harmonic Mean: %s" % harmonic_mean_pixel_value)

        # pixel_array_np = np.array(pixel_array_orig)
        # pixel_array_sort = np.sort(pixel_array_np)
        # pixel_median_value = statistics.median(pixel_array_sort)
        # assert pixel_median_value == int(np.median(pixel_array_orig))
        pixel_median_value = int(np.median(pixel_array))
        #print("Median: %s" % pixel_median_value)

        pixel_variance = np.var(pixel_array, ddof=1) #statistics.variance(pixel_array)
        #print("Variance: %s" % pixel_variance)

        pixel_standard_dev = np.std(pixel_array, ddof=1) #statistics.stdev(pixel_array)
        #print("Stdev: %s" % pixel_standard_dev)

        #  statistics.pstdev(pixel_array_orig) == np.std(pixel_array_orig, ddof=0)
        # pixel_pstandard_dev = statistics.pstdev(pixel_array)
        pixel_pstandard_dev = np.std(pixel_array, ddof=0)
        #print("Pstdev %s" % pixel_pstandard_dev)

        min_pixel = np.min(pixel_array) #pixel_array_sort[0]
        max_pixel = np.max(pixel_array) #pixel_array_sort[-1]
    ...
    result_file_lines.append([
        stock_id,
        np.mean(non_zero_list),
        np.mean(total_pixel_sum_list),
        np.mean(mean_pixel_value_list),
        np.mean(harmonic_mean_pixel_value_list),
        np.mean(pixel_median_value_list),
        np.mean(pixel_variance_list),
        np.mean(pixel_standard_dev_list),
        np.mean(pixel_pstandard_dev_list),
        np.mean(min_pixel_list),
        np.mean(max_pixel_list),
        np.mean(minority_pixel_value_list),
        np.mean(minority_pixel_count_list),
        np.mean(majority_pixel_value_list),
        np.mean(majority_pixel_count_list),
        np.mean(pixel_group_count_list)
    ])
